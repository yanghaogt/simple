...
//	获取一天的结束时间
	 ...
	 */
	 ...
	 /**
	 *获取给定日期的时间段
	 * @param date Date类型的变量
	 */
		public static ShDate  getShDate(Date date){
			//Date today = new Date();
			ShDate shDate = new ShDate();
			Date bToday;
			try {
				bToday = DateUtil.getTheDayOfStartTime(date);//给定日期的一天的开始
				shDate.setBegin(bToday);
			...
			Date eToday;
			try {
				eToday = DateUtil.getTheDayOfEndTime(date);//给定日期的一天的结束
				shDate.setEnd(eToday);
			} catch (ParseException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
...
//		System.out.println(daySub(stringFormatDateTime("2015-06-29 00:00:00"), stringFormatDateTime("2015-06-30 23:59:59")));
	...
	/*	Calendar ca = Calendar.getInstance();
		...
		//ShDate shDate = DateUtil.getShDate(new Date());
		//System.out.println(shDate.getBegin());
		//System.out.println(shDate.getEnd());
/*		System.out.println(DateUtil.dateFormatString(DateUtil.getDay(-1, a)));
		System.out.println(DateUtil.dateTimeFormatString(DateUtil.getDay(-1, a)));
		System.out.println(DateUtil.getShDate(DateUtil.getDay(-1, a)));
*/
		String b = "2017-03-01";
		//System.out.println(DateUtil.stringFormatDate(b));
		System.out.println(DateUtil.daySub(a,DateUtil.stringFormatDate(b)));
