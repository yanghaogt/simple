...
import com.syg.manage.model.entity.IPEntry;
 ...
 * @author luma
 */
public abstract class IPSeekerBase {
	/**
	 * <pre>
	...
	private java.io.File qqFile;
	
	protected abstract String getClassPathFileName();
	...
	public IPSeekerBase() {
		try {
			//QQWry.Dat
			Resource res = new ClassPathResource(getClassPathFileName());
			qqFile = res.getFile();
		} catch (Exception e) {
			e.printStackTrace();
			qqFile = null;
		...
		b3 = new byte[3];
		try {
			ipFile = new RandomAccessFile(qqFile, "r");
		} catch (Exception e) {
...
//	}
	...
	public String getDetailedIpInfo(String ip) {
		String queryIp = ip;
		String ipCountry = getCountry(queryIp);
		if (null == ipCountry)
			ipCountry = " 未知";
		ip += " " + ipCountry;
		String ipArea = getArea(queryIp);
		if (null == ipArea)
			ipArea = "未知";
		else if (" CZ88.NET".equals(ipArea))
			ipArea = " ";
		ip += " " + ipArea;
		return ip;
	}
