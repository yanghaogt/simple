...
public class ShDateEditer extends PropertyEditorSupport{
	 ...
	 */
	public ShDateEditer(DateFormat dateFormat,DateFormat dateFormat2, boolean allowEmpty) {
	 ...
	 */
	public ShDateEditer(DateFormat dateFormat,DateFormat dateFormat2, boolean allowEmpty, int exactDateLength) {
		this.dateFormat = dateFormat;
		this.dateFormat2 = dateFormat2;
		this.allowEmpty = allowEmpty;
	...
	public void setAsText(String text) throws IllegalArgumentException {
		if (this.allowEmpty && !StringUtils.hasText(text)) {
			...
			try {
				setValue(new ShDate(this.dateFormat.parse(text)));
			}
			catch (ParseException ex) {
				try {
					setValue(new ShDate(this.dateFormat2.parse(text)));
				}
