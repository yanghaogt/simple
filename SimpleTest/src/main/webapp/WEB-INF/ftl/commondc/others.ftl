...
<#macro headerSelect v=0 c=0 d=0 ajaxMethod="">
  <!--游戏选择-->
  <input type="hidden" id="gameKindSelect" name="gameKind"  value="${example.gameKind!}" />
  <#if d=1>
  <!--时间段选择-->
  ...
  <!--版本-渠道下拉 S--> 
  <div class="chart_menu ps_rt btn_set fr">
        <ul class="clearfix">
        <#if v=1>
          <li <#if example.gameVersion?exists & example.gameVersion !="--">class="on"</#if>>
          	<#assign gv="版本">
          	<#if example.gameVersion?exists & example.gameVersion !="--">
          		<#assign gv="${example.gameVersion}">
          	</#if>
            <a href="javascript:;">${gv}</a>
            <span class="default_sel none">版本</span>
            <input type="hidden" name="gameVersion" value="<#if example.gameVersion??>${example.gameVersion!}<#else>--</#if>"/>
            <div class="chart_menu_alert none">
              <ul class="option_item">
              	<li class="all">全部</li>
                <#if game_version?exists & game_version?size gt 0>
                <#list game_version as item>
                  <li <#if example.gameVersion?exists & example.gameVersion=item.gameVersion>class="on"</#if>>${item.gameVersion!}</li>
                </#list>  
                </#if>
                </ul>
              <a href="javascript:;" class="chart_alert_confirm btn2" data-method="<#if ajaxMethod !="">${ajaxMethod}<#else>1</#if>">确定</a>
            </div>
          </li>
         </#if>
         <#if c=1>
         <li <#if example.channelId?exists & example.channelId !="--">class="on"</#if>>
     	  <#assign channel="渠道">
      	  <#if example.channelId?exists & example.channelId !="--">
      		  <#assign channel="${example.channelId}">
      	  </#if>
          <a href="javascript:;">${channel}</a>
          <span class="default_sel none">渠道</span>
          <input type="hidden" name="channelId" value="<#if example.channelId??>${example.channelId!}<#else>--</#if>"/>
          <div class="chart_menu_alert none">
              <ul class="option_item">
              <li class="all">全部</li>	
              <#if game_channel?exists & game_channel?size gt 0>
              <#list game_channel as item>
                <li <#if example.channelId?exists & example.channelId=item.channelId>class="on"</#if>>${item.channelId!}</li>
              </#list>  
              </#if>
              </ul>
              <a href="javascript:;" class="chart_alert_confirm btn2" data-method="<#if ajaxMethod !="">${ajaxMethod}<#else>1</#if>">确定</a>
